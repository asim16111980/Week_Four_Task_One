    ...
    return (
        <header className="w-full h-[38px] flex items-center justify-between p-2 relative">
            ...
            <div className='flex-1 flex items-center justify-end gap-1'>
                <button type='button'><FontAwesomeIcon icon={faSearch} onClick={() => setIsSearchOpen(!isSearchOpen)} /></button>
                ...
                <button type="button" className='w-8 h-8 text-xl text-black md:hidden ml-0' onClick={() => setIsMenuOpen(!isMenuOpen)}><FontAwesomeIcon icon={isMenuOpen ? faTimes : faBars} /></button>
            </div>
           
            <nav className={`w-full h-auto md:gap-6 absolute md:static top-[38px] left-0 text-black bg-white transition-all duration-300 ease-in-out shadow  ${isMenuOpen ? "block" : "hidden"
                }`}>
