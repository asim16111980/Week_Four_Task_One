...
const ProductColorOption = ({productId, id, bgColor, onCheck, state }) => {
  return (
    <div className={`relative size-5 rounded-full ${state ? "border-2 border-black" : "border-0"}`} style={{ backgroundColor: state ? "transparent" : bgColor }}>
      ...
      <input id={id} type="radio" name={productId} value={bgColor} className="appearance-none absolute inset-0 z-10 size-full bg-transparent rounded-full" onChange={onCheck} />
    </div>
  )
}
